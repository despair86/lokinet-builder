...
windows: windows-sodium
	cd $(BUILD_DIR) && cmake $(LLARPD_SRC) -DSTATIC_LINK=ON -DSODIUM_LIBRARIES=$(SODIUM_LIB) -DSODIUM_INCLUDE_DIR=$(DEP_PREFIX)/include -DCMAKE_TOOLCHAIN_FILE=$(MINGW_TOOLCHAIN) -DHAVE_CXX17_FILESYSTEM=OFF -DTUNTAP=OFF
	$(MAKE) -C $(BUILD_DIR)
	cp $(BUILD_DIR)/lokinet.exe $(EXE).exe

windows-release: windows-sodium
	cd $(BUILD_DIR) && cmake $(LLARPD_SRC) -DSTATIC_LINK=ON -DSODIUM_LIBRARIES=$(SODIUM_LIB) -DSODIUM_INCLUDE_DIR=$(DEP_PREFIX)/include -DCMAKE_TOOLCHAIN_FILE=$(MINGW_TOOLCHAIN) -DHAVE_CXX17_FILESYSTEM=OFF -DTUNTAP=OFF -DRELEASE_MOTTO="$(shell cat $(MOTTO))"
	$(MAKE) -C $(BUILD_DIR)
	cp $(BUILD_DIR)/lokinet.exe $(EXE).exe
